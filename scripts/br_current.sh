...
[[  "$cb" == "" ]] && /opt/usr/nx-ks/popup_timeout  " [  配置文件不可调整  ] " 2 && exit
br=$(/opt/usr/nx-ks/popup_entry "当前码率 ( Mbps ) :" "应用修改" 取消 $cb number )
[[ "$br" == "" ]]  && exit
[[ "${avail_br[@]}" =~ "${br}" ]] ||  { $(/opt/usr/nx-ks/popup_timeout  " [  值不可用  ] " 3) &&  exit ; }
...
/opt/usr/nx-ks/popup_timeout  " [ 码率已更新  ] " 2
