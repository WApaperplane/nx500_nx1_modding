function Osd(controller) {
    ...
    this.cvs = null;
    this.ctx = null;
    this.touchStartTime = 0;  // 触屏手势:按下时间戳
    this.touchStartX = 0;     // 触屏手势:按下 X
    this.touchStartY = 0;     // 触屏手势:按下 Y
    this.lastTapTime = 0;     // 触屏手势:上次轻点时间
    this.shutterFiring = false; // 快门序列进行中
    this.init();
}
...
Osd.prototype.onMouseDown = function () {
    ...
    function mouseDown(ev, osd, mouse) {
        ...
        if (mouse) {
            ...
            self.touchStartX = pageX;
            self.touchStartY = pageY;
        }
...
/* 手势模式(设置菜单可切换):
 *   off     - 关闭(仅保留拖动对焦点)
 *   focus   - 轻点 = AF 对焦;双击 = 拍照
 *   shutter - 轻点 = 对焦并拍照(双击忽略)
 * 位移超过阈值或按住超过 0.5s 判定为拖动对焦点,不触发轻点。
 */
Osd.prototype.handleTapGesture = function (ev) {
    var self = this;
    if (typeof app === 'undefined' || !app.keyInput) {
        return;
    }
    var gestures = Settings.getTouchGestures();
    if (gestures === 'off') {
        return;
    }
    var t = ev.originalEvent.changedTouches;
    if (!t || t.length === 0) {
        return;
    }
    var now = Date.now();
    var dt = now - self.touchStartTime;
    var dx = t[0].clientX - self.touchStartX;
    var dy = t[0].clientY - self.touchStartY;
    var moved = Math.sqrt(dx * dx + dy * dy);
    if (moved > 14 || dt > 500) {
        // 拖动对焦点或长按,忽略
        return;
    }
    var isDoubleTap = (now - self.lastTapTime) < 300;
...
/* AF 对焦(触发相机 AF_ON 按钮) */
Osd.prototype.fireAfFocus = function () {
    app.keyInput.onKey('AF_ON');
};

/* 快门序列:半按对焦 -> 全按拍照 -> 释放 */
Osd.prototype.fireShutter = function () {
    var self = this;
    if (self.shutterFiring) {
        return;
    }
    self.shutterFiring = true;
    app.keyInput.onKeyDown('S1');
    setTimeout(function () {
        app.keyInput.onKeyDown('S2');
    }, 90);
    setTimeout(function () {
        app.keyInput.onKeyUp('S2');
    }, 190);
    setTimeout(function () {
        app.keyInput.onKeyUp('S1');
        self.shutterFiring = false;
    }, 280);
};
